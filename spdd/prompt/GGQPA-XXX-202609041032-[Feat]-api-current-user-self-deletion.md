...
2. Technical implementation:
   ...
   - Dependency change limited to the Hegel bump `0.5.1 → 0.6.0`. The upgrade is source-compatible for everything this project uses (the only removed API, the `Mode` setting, was never used). 0.6.0's `Stateful.run(machine, tc)` discovers `@Rule` / `@Invariant` methods reflectively (single `TestCase` parameter, ordered by name), enables a random subset of rules per test case (swarm testing), lets the engine pick each next rule, skips a rule whose `assume` fails without counting it as a step, checks every invariant on the initial and final state and samples them in between (probability `1 / stepCount` after each rule, `alwaysRun = true` to check after every rule), and shrinks the failing action sequence like any other draw. A `Pool<T>` records values earlier rules produced; drawing from it through `reusable()` / `consuming()` is a normal draw (replayed and shrunk), and drawing from an empty pool rejects the rule.
   - No new dependencies otherwise. `@HegelTest` is a JUnit `@TestTemplate` driven by a `TestTemplateInvocationContextProvider`, the same mechanism as `@ParameterizedTest`, so it composes with `SpringExtension` (`@Autowired` fields are injected once per test instance; Hegel supplies the `TestCase` parameter per invocation). Operation 13 verifies this first and stops with a report if it does not hold — no silent fallback to a looped `@Test`.
...
3. Business logic and testing:
   ...
   - The **stateful full-stack lifecycle property** covers the same HTTP contract from the other direction: instead of one fixed six-step script it lets Hegel drive random sequences of register / log in / log in with the wrong password / log out / delete self / delete without a session against the running application, with a model of three facts (`registered`, current `password`, the set of `live` tokens among every `issued` cookie) predicting each response. Rules **branch on the model** rather than `assume` — a register while registered must be `409`, a log-in while unregistered must be `401 INVALID_CREDENTIALS`, a delete with a dead cookie must be `401 NOT_LOGGED_IN` — so nearly every engine-chosen step is a real check and only the cookie-drawing rules reject (when nothing has been issued yet). Invariants compare every issued cookie and the email's registration state against the model. The two full-stack properties share one package-private HTTP helper and the same sequence-suffixed `lifecycle.test` emails, and the stateful one also deletes what it created before finishing.
...
5. Test classes: `UserServiceDeletionPropertyTest` (Spring-free), `UserLifecycleIntegrationPropertyTest`, `UserLifecycleStatefulPropertyTest` and `CurrentUserApiTest` (Spring + MockMvc) — plain classes, no base class. `UserLifecycleStatefulPropertyTest` holds a nested static state-machine class (`UserLifecycleMachine`) whose `@Rule` / `@Invariant` methods take a single `TestCase`; `Stateful` drives it. `user.web.UserApi` is a package-private test helper (thin wrapper over `MockMvcTester`) shared by both full-stack properties.
...
6. `UserLifecycleIntegrationPropertyTest` uses `EmailPasswordGenerators.validPasswords()` / `randomizeCase()` (made public) and `MockMvcTester` through `UserApi`.
7. `UserLifecycleStatefulPropertyTest` uses `UserApi`, `EmailPasswordGenerators.validPasswords()` / `randomizeCase()` / `tokensLike()` (made public), and Hegel's `Stateful`, `Rule`, `Invariant`, `Pool`.
...
3. `user.EmailPasswordGenerators`: make the class `public final` and make `validEmails()`, `validPasswords()`, `randomizeCase(TestCase, String)` and `tokensLike()` `public static`; other members unchanged.
...
1. Class: `@SpringBootTest @AutoConfigureMockMvc @ActiveProfiles("test")`; `@Autowired MockMvcTester mvc`; `private static final AtomicLong SEQUENCE = new AtomicLong()`. The HTTP helpers live in the package-private test class `user.web.UserApi` (constructed from the `MockMvcTester`, shared with Operation 13b): `register(email, password)`, `login(email, password)`, `logout(Cookie)`, `deleteMe(Cookie)`, `session(Cookie)` returning `MvcTestResult` (a `null` cookie means "no cookie sent"); static `sessionCookie(result)` as in `SessionApiTest`; `idOf(result)` extracting `$.id` as a `long`; the constant `TOKEN_SHAPE` (`[A-Za-z0-9_-]{43}`). The request-body helper must **JSON-escape** `\` and `"` (valid passwords contain both); fixed-password tests get away with concatenation, a property does not.
...
### Operation 13b — Create Stateful Full-Stack Hegel Property - `user.web.UserLifecycleStatefulPropertyTest`
The same contract as Operation 13, checked the other way round: Hegel chooses the action sequence. Keep Operation 13 exactly as it is — this operation adds, it never replaces.
1. Class: `@SpringBootTest @AutoConfigureMockMvc @ActiveProfiles("test")`; `@Autowired MockMvcTester mvc`; its own `private static final AtomicLong SEQUENCE`. One property `@HegelTest void randomHttpLifecyclesAgreeWithTheModel(TestCase tc)`.
2. Property body: draw `local` from `fromRegex("[a-z0-9]{1,12}").fullmatch(true)` and `password` from `validPasswords()`; `suffix = "." + SEQUENCE.incrementAndGet() + "@lifecycle.test"`, `email = local + suffix`; build `new UserLifecycleMachine(new UserApi(mvc), tc, local, suffix, password)`; call `Stateful.run(machine, tc)`; then `machine.cleanUp()`.
3. Nested `static final class UserLifecycleMachine` — the model: `registered` (boolean, initially false), `password` (the credential a log-in must currently present), `userId` (id of the current registration, `null` when unregistered), `Pool<Cookie> issued` plus a parallel `List<Cookie>` of every cookie ever received, and `Set<String> live` (token values the model considers live). Rules — each a `@Rule` method with one `TestCase` parameter that **branches on the model instead of assuming**, so the negative path is checked, not skipped:
   - `register` — if unregistered: draw a fresh `password` from `validPasswords()`, `POST /api/users` → `201`, `$.email == email`, id differs from the previous `userId` when there was one; set `registered`, `userId`. If registered: the same call with the current password → `409`, `$.code == "EMAIL_ALREADY_REGISTERED"`.
   - `logIn` — email as `randomizeCase(tc, local) + suffix`. If registered: `201`, cookie value matches `TOKEN_SHAPE`, `$.email == email`, `$.id == userId`, `Set-Cookie` contains `HttpOnly`; add the cookie to the pool and to `live`. If unregistered: `401`, `$.code == "INVALID_CREDENTIALS"`, no `Set-Cookie` header.
   - `logInWithTheWrongPassword` — draw `other` from `validPasswords().filter(!equals(password))` → `401 INVALID_CREDENTIALS` whatever the model says, no `Set-Cookie`.
   - `logOut` — draw a cookie from `issued.reusable()` (rejects the rule while nothing was issued) → `204` and the cleared cookie header, whether the cookie is live or dead (logout is idempotent); remove its token from `live`.
   - `deleteSelf` — draw a cookie from `issued.reusable()`. If its token is in `live`: `204`, empty body, `Set-Cookie` starts with `SESSION=;` and contains `Max-Age=0`; then `registered = false`, `userId = null`, `live` cleared (every other device is logged out too). Otherwise: `401`, `$.code == "NOT_LOGGED_IN"`, same cleared-cookie header, model untouched.
   - `deleteWithoutASession` — draw a boolean `forged`; send `DELETE /api/users/me` either with no cookie or with a `SESSION` cookie drawn from `tokensLike()` → `401 NOT_LOGGED_IN` plus the cleared-cookie header.
   Invariants — `@Invariant` methods (default sampling, so their cost stays constant as the step count grows):
   - `issuedCookiesAgreeWithTheModel` — for every issued cookie, `GET /api/session` → `200` with `$.email == email` and `$.id == userId` when its token is in `live`, otherwise `204` with an empty body.
   - `registrationAgreesWithTheModel` — if registered: `POST /api/users` with the current password → `409 EMAIL_ALREADY_REGISTERED`; if not: `POST /api/session` with the current password → `401 INVALID_CREDENTIALS`. Both are side-effect free, so the invariant never changes the state it checks.
   - `cleanUp()` (plain method, called by the test body after `Stateful.run` returns): if registered, log in with the current password (`201`) and delete self (`204`); then every issued cookie → `GET /api/session` `204`, and `POST /api/session` with the current password → `401 INVALID_CREDENTIALS` — the database holds nothing for this email.
4. Constraints: the state machine draws only through `tc` (values, pooled cookies, the forged/no-cookie choice) so a failing sequence replays and shrinks; the case-randomizer is fed only `local`, never the sequence-suffixed email (Operation 13.3). No `testCases` override unless the run exceeds ~10 s locally, then `@HegelTest(testCases = 25)` and a README note. Assertions never assume an empty database; no `@DirtiesContext`, no `@Transactional`, no `@Sql`.
...
4. After the "One property runs the whole stack" bullet add: "**…and a state machine runs it again, in any order.** `UserLifecycleStatefulPropertyTest` checks the same HTTP contract with Hegel's stateful testing: a small class of `@Rule` methods (register, log in, log in with the wrong password, log out, delete self, delete without a session) and `@Invariant` methods, driven by `Stateful.run`. The engine chooses which rules to enable and in which order, a `Pool` hands previously issued cookies back to later rules, and a three-fact model predicts every status code — so a failing run shrinks to the shortest action sequence that breaks the contract."
...
2. Report the wall-clock time of `UserLifecycleIntegrationPropertyTest` and `UserLifecycleStatefulPropertyTest`; apply the `testCases` bound from Operations 13.4 / 13b.4 only if needed, and say so.
...
7. Testing Norms: Spring-free properties build a fresh `ServiceFixture` per body; the full-stack properties use one shared context and clean up their own data; emails in the full-stack properties are sequence-suffixed and live in `lifecycle.test`; the stateful property keeps its model inside the state-machine class, draws only through the `TestCase`, and lets rules branch on the model so that only pool draws ever reject a rule; example-based `@Test` only for `401` wiring and the page content check. No `@DirtiesContext`, no `@Transactional` on tests, no `@Sql` cleanup.
...
2. Performance Constraints: with the `test` profile (BCrypt strength 4) the full `./gradlew test` must stay in the same order of magnitude as before this feature; `UserLifecycleIntegrationPropertyTest` and `UserLifecycleStatefulPropertyTest` should each finish in under ~10 s on a developer machine, otherwise bound the offender with `@HegelTest(testCases = 25)` and document it. Production strength stays 10.
...
4. Integration Constraints: all `@SpringBootTest @AutoConfigureMockMvc` classes activate the `test` profile so they share one context and one H2; both lifecycle properties leave no rows for their emails behind (verified by the final `401 INVALID_CREDENTIALS`); fixed-email tests keep `example.com`, the properties own `lifecycle.test`.
...
7. Technical Constraints: the only Gradle dependency change is `dev.hegel:hegel` `0.5.1 → 0.6.0`, no new dependencies; no JPA cascade or bidirectional `User`↔`Session` mapping; `UserRepository` and `SessionRepository` each gain exactly one method and stay on the bare `Repository` marker; `RegistrationController` unchanged; `SessionController` changes limited to the `clearedSessionCookie()` extraction; Hegel JVM flags in `build.gradle.kts` untouched; no `src/test/resources/application.properties`.
...
| 6 | Full integration property test: register → login → logout → login → delete → re-register same email | Operation 13 (Hegel `@TestTemplate` inside `@SpringBootTest`, steps 1–6); Operation 13b (the same contract under engine-chosen sequences via `Stateful`); Operation 10 keeps both fast; Safeguards 2, 4 |
| 7 | The test finishes by deleting the (re-created) user to avoid interference | Operation 13 cleanup step and Operation 13b `cleanUp()` (login, delete, verify `401`); sequence-suffixed `lifecycle.test` emails |
