...
The login page calls `POST`/`GET`/`DELETE /api/session`; the session travels in an HttpOnly `SESSION` cookie that references a server-side session record. From the logged-in view you can delete your account (`DELETE /api/users/me`); that removes the user and all of its sessions, and the email can be registered again. The database is in-memory H2, so registered users and sessions are lost when the app restarts.
...
- **Sessions follow the same recipe.** Login sessions live behind a second narrow repository (`SessionRepository`, four methods) with its own hand-written in-memory fake. Time comes from an injected `java.time.Clock`, so session expiry is a deterministic property rather than a flaky sleep, and the login/logout/query state machine is checked with a model-based Hegel property that replays random action sequences against a tiny set of "live tokens".
- **One property runs the whole stack.** `UserLifecycleIntegrationPropertyTest` is a `@HegelTest` inside a `@SpringBootTest`: it drives register → login → logout → login → delete → re-register through MockMvc with drawn credentials and cleans up after itself, so it also proves the deletion order against the real foreign key. API tests run with the `test` profile, which lowers BCrypt to strength 4 via `app.password.bcrypt-strength`.
- **…and a state machine runs it again, in any order.** `UserLifecycleStatefulPropertyTest` checks the same HTTP contract with Hegel's stateful testing: a small class of `@Rule` methods (register, log in, log in with the wrong password, log out, delete self, delete without a session) and `@Invariant` methods, driven by `Stateful.run`. The engine chooses which rules to enable and in which order, a `Pool` hands previously issued cookies back to later rules, and a three-fact model predicts every status code — so a failing run shrinks to the shortest action sequence that breaks the contract.
